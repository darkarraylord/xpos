...
class SM_XPos_Block_Adminhtml_Till_Grid extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _prepareCollection() {
        ...
        if(Mage::helper('xpos/configXPOS')->getIntegrateXmwhEnable()==1){
            ...
            if(Mage::helper('xpos/configXPOS')->getIntegrateXmwhEnable()==2){
                $collection->getSelect()->joinLeft(
                    array('warehouseTable'=>$collection->getTable('inventoryplus/warehouse')),
                    '`main_table`.`warehouse_id` = `warehouseTable`.`warehouse_id`',array('warehouse_name')
                );
            }
        }
    ...
    protected function _prepareColumns() {
        ...
        $this->addColumn('store_id', array(
            'header' => Mage::helper('xpos')->__('Store'),
            'align' => 'left',
            'index' => 'store_id',
            'renderer' => 'xpos/adminhtml_till_render_storeForTill'
        ));
